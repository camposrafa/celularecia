<?php
	...
	if( isset($_POST) && !empty($_POST) ){
		...
		$resultado = $bd->query("UPDATE orcamento SET
									NOME='$cNome',
									TELEFONE='$cTelefone',
									APARELHO='$cAparelho',
									CONSERTO='$cConserto',
		...
		if($resultado){
			echo
			"<script>
			alert('Orçamento do cliente ".utf8_encode($cNome)." foi atualizado com sucesso!'); 
...
<html lang="pt-BR">
	<head>
		...
		<title>Atualizar Orçamento - Celular & Cia</title>
			  ...
			  <div class="panel-body">
				<form method="POST">
					...
					<label for="campoNome"><b>Nome</b></label>
						<input type="text" name="campoNome" class="form-control" placeholder="Digite o nome..." required="required" value="<?php echo $os["NOME"];?>"/>
					</div>
					<div class="form-group">
						<label for="campoTelefone"><b>Telefone</b></label>
						<input type="text" name="campoTelefone" class="form-control" placeholder="Ex.: (14) 98765-4321" required="required" value="<?php echo $os["TELEFONE"];?>"/>
					</div>
					<div class="form-group">
						<label form="campoAparelho"><b>Aparelho</b></label>
						<input type="text" name="campoAparelho" class="form-control" placeholder="Ex.: Celular, notebook" required="required" value="<?php echo $os["APARELHO"];?>"/>
					</div>
					<div class="form-group">
						<label form="campoConserto"><b>Conserto</b></label>
						<input type="text" name="campoConserto" class="form-control" placeholder="Ex.: Troca de tela" required="required"value="<?php echo $os["CONSERTO"];?>" />
					</div>
					<div class="form-group">
						<label form="campoValor"><b>Valor</b></label>
						<input type="text" name="campoValor" class="form-control" placeholder="Ex.: R$100,00" required="required" value="<?php echo $os["VALOR"];?>"/>
					</div>
					<div class="form-group">
						<label form="campoObservacao"><b>Observação</b></label>
						<input type="text" name="campoObservacao" class="form-control" placeholder="Ex.: Data específica" value="<?php echo $os["OBSERVACAO"];?>"/>
					</div>
