<?php
	...
	if( empty($_GET) || !isset($_GET["codigo"]) ){
		...
		header("Location: relatorioOS.php");
		exit;
	...
	@$codigo = $_GET["codigo"];
	$os = $bd->query("select * from os where CODIGO=$codigo");
	if($os && $os->num_rows==1){
	...
	if( isset($_POST) && !empty($_POST) ){
		...
		$resultado = $bd->query("UPDATE os SET
									NOME='$cNome',
									TELEFONE='$cTelefone',
									APARELHO='$cAparelho',
									CONSERTO='$cConserto',
		...
		if($resultado){
			echo
			"<script>
			alert('Ordem de serviço do cliente ".utf8_encode($cNome)." foi atualizada com sucesso!'); 
			location= './relatorioOS.php';
			</script>";
			exit;
		}else{
			echo
...
<html lang="pt-BR">
	<head>
		...
		<title>Atualizar OS - Celular & Cia</title>
