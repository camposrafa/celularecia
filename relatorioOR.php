...
<head>
	...
	<nav class="navbar navbar-dark bg-dark">
		...
		<div class="jumbotron">
			<div class="container">
				<center><h4><p>Visualização de orçamentos</p></h4></center>
			</div>
		</div>
		...
		<div class="container">
			<table class="table table-hover table-striped">
				...
				<?php
					...
					$orcamento = $bd->query("select * from orcamento");
					
					# Verificar se localizou algum cliente...
					if($orcamento && $orcamento->num_rows>0){
						while($or = $orcamento->fetch_assoc()){
							echo'
							<tr>
								<td align="center"><strong>'.$or["CODIGO"].'</strong></td>
								<td>'.utf8_encode($or["NOME"]).'</td>
								<td align="left">'.utf8_encode($or["TELEFONE"]).'</td>
								<td align="left">'.utf8_encode($or["APARELHO"]).'</td>
								<td align="left">'.utf8_encode($or["CONSERTO"]).'</td>
								<td align="left">'.utf8_encode($or["VALOR"]).'</td>
								<td align="left">'.utf8_encode($or["OBSERVACAO"]).'</td>
								<td align="left">
									<div class="btn-group" role="group" aria-label="Basic example">
										<a title="Alterar" class="btn btn-warning" href="alteraOR.php?codigo='.$or["CODIGO"].'">✎</a>
										<a title="Excluir" class="btn btn-danger" href="excluirOR.php?codigo='.$or["CODIGO"].'">☠</a>
									</div>
								</td>
							</tr>';
						}
